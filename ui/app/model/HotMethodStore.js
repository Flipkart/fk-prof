...
export default class HotMethodStore {
  ...
  flatten(nodes, topLevel, bodyRoot) {
    ...
    deDupNodes.forEach(([k, v]) => {
      ...
      if (topLevel) {
        this.flatten(nodes, false, false);
      } else {
        v[2].filter(([k, v]) => v['chld']).forEach(([k, v]) => {
          if (v['chld'].length === 1) {
            v['chld'][0]['data'][2] = v['data'][2];
            Object.assign(nextLayerNodes, v['chld'])
          } else {
            console.log('hotMethod node should never happen more than one child/hotMethod parent, this has : ', v['child']);
          }
        });
        this.nodes[vIndex][1] = this.flatten(nextLayerNodes, false);
  ...
  getChildrenAsync(uniqueId) {
    ...
    //else fetch from api
    const body = (uniqueId === -1) ? [] : this.nodes[uniqueId][2];    //body contains the parts of the aggregated node at uniqueId
