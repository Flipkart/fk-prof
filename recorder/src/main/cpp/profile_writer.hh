...
public:
    ...
    virtual void write_unbuffered(const std::uint8_t *data, std::uint32_t sz,
...
private:
    ...
    template <class T>
    std::uint32_t ensure_freebuff(const T &value);

    void write_unchecked(std::uint32_t value);

    template <class T>
    void write_unchecked_obj(const T &value);
...
struct TruncationThresholds {
    ...
    TruncationThresholds(TruncationCap _cpu_samples_max_stack_sz = DEFAULT_MAX_FRAMES_TO_CAPTURE,
                         TruncationCap _io_trace_max_stack_sz = DEFAULT_MAX_FRAMES_TO_CAPTURE)
        : cpu_samples_max_stack_sz(_cpu_samples_max_stack_sz),
          io_trace_max_stack_sz(_io_trace_max_stack_sz) {
    }
...
private:
    ...
    CtxId next_ctx_id;
    std::unordered_map<uiid_t, FdId> known_fds;
    FdId next_fd_id;
    ...
    MthId report_new_mthd_info(const char *file_name, const char *class_name,
                               const char *method_name, const char *method_signature,
                               const BacktraceType bt_type);

    CtxId report_new_ctx(const std::string &name, const bool is_generated,
                         const std::uint8_t coverage_pct, const PerfCtx::MergeSemantic m_sem);

    void report_new_ctx(const CtxId ctx_id, const std::string &name, const bool is_generated,
                        const std::uint8_t coverage_pct, const PerfCtx::MergeSemantic m_sem);

    /* helper methods to populate proto structures */
    void populate_ctx_in_ss(recording::StackSample *const ss,
                            const PerfCtx::ThreadTracker::EffectiveCtx &ctx, std::uint8_t ctx_len,
                            bool default_ctx);

    void populate_bt_in_ss(recording::StackSample *const ss, const Backtrace &bt,
                           std::uint32_t frames_to_write, bool snipped);

    void populate_fdread(recording::IOTrace *const evt, const blocking::FdReadEvt &read_evt,
                         blocking::EvtType type);

    void populate_fdwrite(recording::IOTrace *const evt, const blocking::FdWriteEvt &write_evt,
                          blocking::EvtType type);
...
public:
    ProfileSerializingWriter(jvmtiEnv *_jvmti, ProfileWriter &_w,
                             SiteResolver::MethodInfoResolver _fir,
                             SiteResolver::LineNoResolver _lnr, PerfCtx::Registry &_reg,
                             const FlushCtr _sft, const TruncationThresholds &_trunc_thresholds,
    ...
    virtual MthId recordNewMethod(const jmethodID method_id, const char *file_name,
                                  const char *class_name, const char *method_name,
                                  const char *method_signature) override;
