...
#ifndef CIRCULAR_QUEUE_1_H
#define CIRCULAR_QUEUE_1_H
...
protected:
    virtual void write(TraceType &entry, InMsg &in_msg) = 0;
};

namespace cpu {

struct TraceInfo {
    JVMPI_CallFrame* java_trace;
...
struct InMsg {
    ...
    InMsg(JVMPI_CallFrame* trace, std::size_t frame_count, ThreadBucket *info,
          const BacktraceError error, const bool default_ctx);
...
protected:
    void write(Sample &entry, InMsg &in_msg) override;
};
}
}

extern template class fk_bench::CircularQueue<fk_bench::cpu::Sample, fk_bench::cpu::InMsg>;
