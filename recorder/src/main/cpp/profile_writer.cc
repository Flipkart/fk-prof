...
template <class T>
inline std::uint32_t ProfileWriter::ensure_freebuff(const T &value) {
    auto sz = value.ByteSize();
    return ensure_freebuff(sz + MIN_FREE_BUFF);
}

void ProfileWriter::write_unchecked(std::uint32_t value) {
    auto _data = google::protobuf::io::CodedOutputStream::WriteVarint32ToArray(
        value, data.buff + data.write_end);
    data.write_end = _data - data.buff;
}

template <class T>
void ProfileWriter::write_unchecked_obj(const T &value) {
    auto sz = value.ByteSize();
    write_unchecked(sz);
...
ProfileWriter::ProfileWriter(std::shared_ptr<RawWriter> _w, Buff &_data)
    : w(_w), data(_data), header_written(false) {
...
void ProfileSerializingWriter::report_new_ctx(const ProfileSerializingWriter::CtxId ctx_id,
                                              const std::string &name, const bool is_generated,
                                              const std::uint8_t coverage_pct,
                                              const PerfCtx::MergeSemantic m_sem) {
    ...
    if (!is_generated) {
        ...
        new_ctx->set_merge(
            static_cast<recording::TraceContext_MergeSemantics>(static_cast<int>(m_sem)));
    }
    new_ctx->set_trace_name(name);
    s_c_new_ctx_info.inc();
}

ProfileSerializingWriter::CtxId
ProfileSerializingWriter::report_new_ctx(const std::string &name, const bool is_generated,
                                         const std::uint8_t coverage_pct,
                                         const PerfCtx::MergeSemantic m_sem) {
...
ProfileSerializingWriter::CtxId ProfileSerializingWriter::report_ctx(PerfCtx::TracePt trace_pt) {
    ...
    if (known_ctx == known_ctxs.end()) {
        ...
        SPDLOG_DEBUG(logger, "Reporting trace named '{}', cov {}% as ctx-id: {}", name,
                     coverage_pct, ctx_id);
        return ctx_id;
    } else {
...
ProfileSerializingWriter::ThdId
ProfileSerializingWriter::report_thd_info(const ThreadBucket *const thd_bucket) {
...
void ProfileSerializingWriter::populate_ctx_in_ss(recording::StackSample *const ss,
                                                  const PerfCtx::ThreadTracker::EffectiveCtx &ctx,
                                                  std::uint8_t ctx_len, bool default_ctx) {
...
void ProfileSerializingWriter::populate_bt_in_ss(recording::StackSample *const ss,
                                                 const Backtrace &bt, std::uint32_t frames_to_write,
                                                 bool snipped) {
    if (snipped)
        s_c_frame_snipped.inc();
...
void ProfileSerializingWriter::record(const cpu::Sample &entry) {
    ...
    auto frames_to_write =
        Util::min((std::uint32_t)trace.num_frames, trunc_thresholds.cpu_samples_max_stack_sz);
    populate_bt_in_ss(ss, trace, frames_to_write, snipped);

    s_m_cpu_sample_add.mark();
}

void ProfileSerializingWriter::populate_fdread(recording::IOTrace *const evt,
...
void ProfileSerializingWriter::populate_fdwrite(recording::IOTrace *const evt,
...
void ProfileSerializingWriter::record(const iotrace::Sample &entry) {
    ...
    auto frames_to_write =
        Util::min((std::uint32_t)entry.trace.num_frames, trunc_thresholds.io_trace_max_stack_sz);
...
void ProfileSerializingWriter::fill_frame(const NativeFrame &pc, recording::Frame *f) {
    ...
    if (it == std::end(known_symbols)) {
        ...
        if (it == std::end(known_symbols)) {
            my_id = report_new_mthd_info(file_name.c_str(), "", fn_name.c_str(), "",
...
ProfileSerializingWriter::MthId ProfileSerializingWriter::report_new_mthd_info(
    const char *file_name, const char *class_name, const char *method_name,
    const char *method_signature, const BacktraceType bt_type) {
...
ProfileSerializingWriter::MthId
ProfileSerializingWriter::recordNewMethod(const jmethodID method_id, const char *file_name,
                                          const char *class_name, const char *method_name,
                                          const char *method_signature) {
    auto my_id = report_new_mthd_info(file_name, class_name, method_name, method_signature,
                                      BacktraceType::Java);
...
ProfileSerializingWriter::ProfileSerializingWriter(jvmtiEnv *_jvmti, ProfileWriter &_w,
                                                   SiteResolver::MethodInfoResolver _fir,
                                                   SiteResolver::LineNoResolver _lnr,
    ...
    : jvmti(_jvmti), w(_w), fir(_fir), lnr(_lnr), reg(_reg),
      ...
      s_c_total_mthd_info(
          get_metrics_registry().new_counter({METRICS_DOMAIN, METRIC_TYPE, "mthd_rpt", "total"})),
      s_c_new_mthd_info(
          get_metrics_registry().new_counter({METRICS_DOMAIN, METRIC_TYPE, "mthd_rpt", "new"})),
      s_c_new_pc(
          get_metrics_registry().new_counter({METRICS_DOMAIN, METRIC_TYPE, "pc_rpt", "new"})),

      s_c_bad_lineno(
          get_metrics_registry().new_counter({METRICS_DOMAIN, METRIC_TYPE, "line_rpt", "bad"})),

      s_c_frame_snipped(
          get_metrics_registry().new_counter({METRICS_DOMAIN, METRIC_TYPE, "backtrace_snipped"})),

      s_m_stack_sample_err(get_metrics_registry().new_meter(
          {METRICS_DOMAIN, METRIC_TYPE, "cpu_sample", "err"}, "rate")),
      s_m_cpu_sample_add(get_metrics_registry().new_meter(
          {METRICS_DOMAIN, METRIC_TYPE, "cpu_sample", "rpt"}, "rate")) {
    ...
    report_new_ctx(DEFAULT_CTX_ID, DEFAULT_CTX_NAME, false, _noctx_cov_pct,
                   PerfCtx::MergeSemantic::to_parent);
...
ProfileSerializingWriter::~ProfileSerializingWriter() {
    SPDLOG_TRACE(logger, "Destructor: ProfileSerializingWriter");
    std::vector<PerfCtx::TracePt> user_ctxs;
    reg.user_ctxs(user_ctxs);
    auto next_ctx_to_be_reported = next_ctx_id;
    for (auto pt : user_ctxs)
        report_ctx(pt);

    if ((ser_flush_ctr != 0) || (next_ctx_to_be_reported != next_ctx_id)) {
        SPDLOG_TRACE(logger, "flushing before deleting serializing writer");
        flush();
    }
