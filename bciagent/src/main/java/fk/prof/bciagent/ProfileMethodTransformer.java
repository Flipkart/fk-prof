...
public class ProfileMethodTransformer implements ClassFileTransformer {
  ...
  private static native void bciFailed(String clazzName);
...
//    hooks.methods.put("available()I", sock_input_default);
  ...
  private static void instrument_fileStream_open_mexit(CtMethod m) throws Exception {
    ...
    jStr += "if(" + fdLocalVar + " >= 0) { fk.prof.trace.IOTrace.File.open(" + fdLocalVar + ", this.path, " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + "); }";
  ...
  private static void instrument_fileStream_read_mexit(CtMethod m) throws Exception {
    String jStr = "";
    jStr += codebuild_fileStream_saveFDToLocalVar();
    jStr += "fk.prof.trace.IOTrace.File.read(" + fdLocalVar + ", $_, " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ")";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_fileStream_write_mexit(CtMethod m, int variant) throws Exception {
    ...
    jStr += "fk.prof.trace.IOTrace.File.write(" + fdLocalVar + ", " + count + ", " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ")";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_sockStream_input_mexit(CtMethod m, CtClass socketTimeoutExceptionClass) throws Exception {
    ...
    jStr += "fk.prof.trace.IOTrace.Socket.read(" + fdLocalVar + ", $_, " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ", " + timedoutLocalVar + ")";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_sockStream_output_mexit(CtMethod m) throws Exception {
    String jStr = "";
    jStr += codebuild_sockStream_saveFDToLocalVar();
    jStr += "fk.prof.trace.IOTrace.Socket.write(" + fdLocalVar + ", $3 - $2," + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ");";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_sock_connect_mexit(CtMethod m) throws Exception {
    String jStr = "";
    jStr += codebuild_sockStream_saveFDToLocalVar();
    jStr += "if(" + fdLocalVar + " >= 0) { fk.prof.trace.IOTrace.Socket.connect(" + fdLocalVar + ", $1.toString(), " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + "); }";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_sock_accept_mexit(CtMethod m) throws Exception {
    ...
    jStr += "if(" + fdLocalVar + " >= 0) { fk.prof.trace.IOTrace.Socket.accept(" + fdLocalVar + ", $_.getInetAddress().toString(), " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + "); }";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_ioutil_read_mexit(CtMethod m) throws Exception {
    ...
    jStr += "fk.prof.trace.IOTrace.Socket.read(" + fdLocalVar + ", $_, " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ", false)";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_ioutil_write_mexit(CtMethod m) throws Exception {
    String jStr = "";
    jStr += codebuild_ioUtil_saveFDToLocalVar();
    jStr += "fk.prof.trace.IOTrace.Socket.write(" + fdLocalVar + ", $_," + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + ");";
    m.insertAfter(jStr, true);
  }
  ...
  private static void instrument_sock_ch_cexit(CtConstructor c) throws Exception {
    String jStr = "";
    jStr += codebuild_sockCh_saveFDToLocalVar();
    jStr += "if(" + fdLocalVar + " >= 0) { fk.prof.trace.IOTrace.Socket.accept(" + fdLocalVar + ", $3.toString(), " + elapsedLocalVar + ", " + codebuild_calculateElapsedMillis_expr() + "); }";
    c.insertAfter(jStr, true);
  }

  private static String codebuild_calculateElapsedMillis_expr() {
    return "System.currentTimeMillis() - " + elapsedLocalVar;
  }

  private static String codebuild_fileStream_saveFDToLocalVar() {
    String jStr = "";
    jStr += "int " + fdLocalVar + " = this.fd == null ? -1 : fk.prof.bciagent.FdAccessor.getFd(this.fd);";
    return jStr;
  }

  private static String codebuild_sock_accept_saveFDToLocalVar() {
    String jStr = "";
    jStr += "int " + fdLocalVar + " = ($_ == null || $_.getImpl() == null || $_.getImpl().getFileDescriptor() == null) ? -1 : fk.prof.bciagent.FdAccessor.getFd($_.getImpl().getFileDescriptor());";
    return jStr;
  }

  private static String codebuild_sockStream_saveFDToLocalVar() {
    String jStr = "";
    jStr += "int " + fdLocalVar + " = (this.impl == null || this.impl.getFileDescriptor() == null) ? -1 : fk.prof.bciagent.FdAccessor.getFd(this.impl.getFileDescriptor());";
    return jStr;
  }

  private static String codebuild_ioUtil_saveFDToLocalVar() {
    String jStr = "";
    jStr += "int " + fdLocalVar + " = $1 == null ? -1 : fk.prof.bciagent.FdAccessor.getFd($1);";
    return jStr;
  }
